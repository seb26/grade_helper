...
class CDLLib {
    ...
    parse_csv(csv_data, source_file_name) {
        var color_items = [];
        const CSV_COL_NAMES_NAME = [ 'Clip Identifier', ];
        ...
        csv.data.forEach( (csv_line) => {
            ...
            if ( entry['_sop'] && entry['_sat'] ) {
                ...
                if ( sop_split.length > 1 ) {
                    // TBC FOR NOW: Just keep only the first line of SOP
                    // FUTURE: Use CDL addition functionality to combine the two lines into a single SOP.
