...
class ColorCorrection {
    ...
    get_offset(places=false) {
        return get_string_from_float_rgb( this.color.Offset, places );
    }
    ...
    get_sop_as_string(places=false, pad_negative_values=false) {
        ...
        if ( pad_negative_values ) {
            var c = {
                'Slope': ( this.get_slope(places) >= 0 ? ' ' + this.get_slope(places) : this.get_slope(places) ),
                'Offset': ( this.get_offset(places) >= 0 ? ' ' + this.get_offset(places) : this.get_offset(places) ),
                'Power': ( this.get_power(places) >= 0 ? ' ' + this.get_power(places) : this.get_power(places) ),
            };
        }
        else {
            var c = {
                'Slope': this.get_slope(places),
                'Offset': this.get_offset(places),
...
function get_string_from_float_rgb(float_rgb, places=false) {
    var string = '';
    for ( let i = 0; i < float_rgb.length; i++ ) {
        string += get_string_zero_padded_from_float(float_rgb[i], places) + ' ';
    }
