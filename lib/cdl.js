...
const COLOR_DISPLAY_FLOATING_POINT = 2;
...
function get_float_rgb_from_string(str) {
    ...
    for ( i = 0; i < str.split(' ').length; i++ ) {
        values.push( parseFloat(parts[i]).toFixed(COLOR_DISPLAY_FLOATING_POINT) );
    }
    return [ values[0], values[1], values[2] ];
}
function get_string_zero_padded_from_float(float) {
    return Number.parseFloat(float).toFixed(COLOR_DISPLAY_FLOATING_POINT);
}
